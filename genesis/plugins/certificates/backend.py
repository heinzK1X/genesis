...
class CertControl(Plugin):
	...
	def upload_cert(self, cert, chain, key):
		# Save a certificate and key passed through
		cfg = ConfigParser.ConfigParser()
		if chain:
			c = open('/etc/ssl/certs/genesis/'+name+'.crt', 'w')
			c.write(cert+'\n'+chain)
			c.close()
		else:
			c = open('/etc/ssl/certs/genesis/'+name+'.crt', 'w')
			c.write(cert)
			c.close()
		k = open('/etc/ssl/private/genesis/'+name+'.key', 'w')
		k.write(key)
		k.close()
